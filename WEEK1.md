...
# Workshop material - Day #1 - Git/VS Code
  * Setting up git
    ...
    * Open https://github.com/HexaInnovLab/ftp02, and look at the WEEK1.md
  ...
  * Git (setup/checkout/clone/pull/push)
    ...
    * `git config --global -l`
    * Open https://github.com/HexaInnovLab/ftp02
      * look at source code organization
    * Go back to gitbash and clone the project
    * `git clone git@github.com:HexaInnovLab/ftp02.git`
    * `cd ftp02`
    * `git status`
  * open Visual Studio Code
    * Open folder c:\users\Hvuser\workspace\ftp02
...
# Workshop material - Day #2 - MySQL Workbench/Cli/Curl/Browser to interact w/ application
  ...
  * `CREATE DATABASE FTP02;` and click the lightning button
  * `CREATE USER 'FTP02'@'localhost' IDENTIFIED BY 'FTP02';`
  * `GRANT ALL ON FTP02.* TO 'FTP02'@'localhost';`
  ...
  * go to gitbash, ensure you are in workspace/ftp02
  * `cd restservice/leavemanager`
  * `mvn compile`
  * `mvn exec:java -Dexec.mainClass=com.hexaware.ftp02.util.CliMain`
  ...
  * Build the war file as follows:
    ...
    * `cp target/ftp02-0.0.1-SNAPSHOT.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/ftp02.war`
    * start tomcat and tail its logs
      * `cd D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16`
      * `rm -rf logs/*`
      * `./bin/startup.sh`
      * `tail -f logs/*`
      * `curl -vvv http://localhost:8080/ftp02/api/employees | python -m json.tool`
      * `curl -vvv http://localhost:8080/ftp02/api/employees/2000 | python -m json.tool`
   * `cd ../../webui/lm-app/`
   * `npm install`
   * `ng build`
   * `cp ../../restservice/leavemanager/target/ftp02-0.0.1-SNAPSHOT.war ./ftp02.war`
   * `cd dist`
   * `jar -uvf ../ftp02.war *`
   * `cd ..`
   * `jar -tvf ftp02.war`
   * `cp ftp02.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/ftp02.war`
   * Notice that the tail terminal shows that the new version of the web application archive is now getting deployed
   * Open Chrome browser and navigate to http://localhost:8080/ftp02/
